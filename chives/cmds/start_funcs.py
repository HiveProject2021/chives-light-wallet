...
from chives.cmds.passphrase_funcs import get_current_passphrase
from chives.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from chives.util.keychain import KeyringMaxUnlockAttempts
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    chives = sys.argv[0]
    process = subprocess.Popen(f"{chives} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
