...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-2] == config["farmer"].get("xcc_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-2] == config["pool"].get("xcc_target_address"):
                stop_searching_for_pool = True
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if chives_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return chives_init(root_path, fix_ssl_permissions=fix_ssl_permissions)
...
def chives_init(root_path: Path, *, should_check_keys: bool = False, fix_ssl_permissions: bool = False):
    """
