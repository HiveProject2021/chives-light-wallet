...
class FullNodeRpcApi:
    ...
    async def get_puzzle_and_solution(self, request: Dict) -> Optional[Dict]:
        ...
        self.service.log.warning(f"request:{request}")
        self.service.log.warning(f"coin_record:{coin_record}")
        if coin_record is None:
            raise ValueError(f"coin_record can not be None request:{request}")
        if not coin_record.spent:
            raise ValueError(f"coin_record.spent must spent. coin record {coin_record}")
        if coin_record.spent_block_index != height:
            raise ValueError(f"coin_record.spent_block_index must equal {coin_record.spent_block_index}, you input {height}. coin record {coin_record}")
