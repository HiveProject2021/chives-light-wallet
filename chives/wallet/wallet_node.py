...
from chives.full_node.weight_proof import chunks
...
from chives.types.weight_proof import WeightProof, SubEpochData
from chives.util.byte_types import hexstr_to_bytes
from chives.util.ints import uint32, uint64
from chives.util.keychain import KeyringIsLocked
...
from chives.wallet.util.wallet_sync_utils import (
    validate_additions,
    validate_removals,
...
class WalletNode:
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSChivesConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    def on_disconnect(self, peer: WSChivesConnection):
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSChivesConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSChivesConnection) -> Optional[HeaderBlock]:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSChivesConnection, peak: HeaderBlock
    ...
    async def untrusted_sync_to_peer(
        self, peer: WSChivesConnection, weight_proof: WeightProof, syncing: bool, fork_height: int
    ):
        assert self.wallet_state_manager is not None
