...
export default function WalletSend(props: SendCardProps) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send chives to coloured address. Please enter a chives address.`);
    }
    ...
    const response = await sendTransaction({
      walletId,
      address,
      amount: Number.parseFloat(chives_to_mojo(amount)),
      fee: Number.parseFloat(chives_to_mojo(fee)),
